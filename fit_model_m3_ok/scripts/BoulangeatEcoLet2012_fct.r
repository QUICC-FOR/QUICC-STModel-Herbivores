#------------------------------------------------------------------------------------------
# function to compute co-occurrence based indices
#------------------------------------------------------------------------------------------

comEffects <- function(sp, comOBS, comPRED, comOBS_abundance){

N = nrow(comOBS)
S = ncol(comOBS)

# focal species index
sp.index = which(colnames(comOBS)==sp)
...
# number of observations of the focal species
Nj <- sum(comOBS[,sp])
...
for(k in 1:S){
  ...
  #competitions:
  # potential cases (both species are expected)
  sumC <- sum(comPRED[,k]*comPRED[,sp]) 
     if( sumC ==0 ){  # to test if sum(C) !=0
      Cjk[k]=0
      Ckj[k]=0
      }else{ 
      # both species are expected but only one is observed
      cjk = sum( (!as.logical(comOBS[,k]) & as.logical(comOBS[,sp])) & (comPRED[,k]*comPRED[,sp])==1 ) 
    	ckj = sum( (!as.logical(comOBS[,sp]) & as.logical(comOBS[,k])) & (comPRED[,k]*comPRED[,sp])==1 ) 
      Cjk[k] = cjk / sumC
    	Ckj[k] = ckj / sumC
      }
      
   #facilitation:
   # potential cases (only one species is expected)
   sumF <- sum(xor(as.logical(comPRED[,k]), as.logical(comPRED[,sp]) )) 
     if(sum((comPRED[,k]+comPRED[,sp])==1) ==0 ){   # to test if sum(F) !=0
      Fjk[k]=0
      Fkj[k]=0
      }else{ 
      # both species are observed but only one was expected
      fjk = sum( (comOBS[,k]*comOBS[,sp])==1 & ( as.logical(comPRED[,sp]) & !as.logical(comPRED[,k]) ) ) 
    	fkj = sum( (comOBS[,k]*comOBS[,sp])==1 & ( !as.logical(comPRED[,sp]) & as.logical(comPRED[,k]) ) ) 
      Fjk[k] = fjk /  sumF
    	Fkj[k] = fkj /  sumF
      }    
...
Cxjk = as.vector( (as.matrix(comOBS_abundance) %*% Cjk)  )
Cxkj = as.vector( (as.matrix(comOBS_abundance) %*% Ckj)  )
Fxjk = as.vector( (as.matrix(comOBS_abundance) %*% Fjk)  )
Fxkj = as.vector( (as.matrix(comOBS_abundance) %*% Fkj)  )


names(Cxjk) <- rownames(comOBS_abundance)
names(Cxkj) <- rownames(comOBS_abundance)
names(Fxjk) <- rownames(comOBS_abundance)
names(Fxkj) <- rownames(comOBS_abundance)

return(list(Cjk=Cxjk, Ckj=Cxkj, Fjk=Fxjk, Fkj=Fxkj))
