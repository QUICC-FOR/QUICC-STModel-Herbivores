...
#load("../estimated_params/fine_veget_m3_v2")
...
invD = apply(cbind(invT1, invT2, invT3), 1, function(x){x[which.max(abs(x))]})
invC = apply(cbind(invB1, invB2, invB3), 1, function(x){x[which.max(abs(x))]})
...
par(mar=c(5,5,0,2))
image(tpseq,ppseq*1000,Z,xlab = "Mean annual temperature", ylab = "Annual precipitation (mm)", cex.lab = 1.5, cex.axis = 1.25, col = colo, breaks = c(0:4))#grey(c(0:3)/3))
